...
public abstract class AbstractOrientDBTest implements JdbcSource {
  protected final String TEMP_DB_DIR = "C:\\Users\\.temp";
  protected final String BERKLEY_DB_DIR = Paths.get(TEMP_DB_DIR).toAbsolutePath().normalize() + FILE_SEPARATOR + "orient_db_test";
  protected BerkleyDB berkleyDB;

  protected final String DB_URL = "jdbc:orient:remote:localhost/OSystem";
  protected OrientJdbcConnection dbConnection;

  protected FBaseConfig fBaseConfig;
  protected FBase fBase;
  protected FStore fStore;

  private final String tableNameRandom = "orient_db_table_test_random";
  private final String tableNameAsh = "orient_db_table_test_ash";
  ...
  public void initBackendAndLoad() {
    try {
      ...
      dbConnection = (OrientJdbcConnection) DriverManager.getConnection(DB_URL, "root", "rootpwd");
    } catch (Exception e) {
      log.catching(e);
